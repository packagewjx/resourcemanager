...
func parseResult(fileName string) *MemRecordResult {
	...
	for line, err = reader.ReadString('\n'); err == nil || (err == io.EOF && line != ""); line, err = reader.ReadString('\n') {
		...
		r := res.ThreadTrace[int(tid)]
		if r == nil {
			r = algorithm.FullTraceCalculator()
			res.ThreadTrace[int(tid)] = r
		}
		r.Update([]uint64{addr})
	}
...
func (p *PerfRecorder) record(ctx context.Context, perfCommand *exec.Cmd) <-chan *MemRecordResult {
